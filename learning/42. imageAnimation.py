...
class AnimatedButton(ctk.CTkButton):
    ...
    def import_folders(self, dark_path, light_path):
        image_paths = []
...
class ForeverAnimate(ctk.CTkButton):
    def __init__(self, parent, light_path, dark_path):
        ...
        self.frame_index = 0
        self.animation_length = len(self.frames) - 1
        self.animation_status = ctk.StringVar(value = 'forward')

        self.animation_status.trace('w', self.infinite_animate)

        super().__init__(parent, text = 'An animated button', image = self.frames[self.frame_index], command = self.infinite_animate)
        self.pack(expand = True)
...
AnimatedButton(window, 'assets\imageAnimations\\yellow', 'assets\imageAnimations\\black')
ForeverAnimate(window, 'assets\imageAnimations\\yellow', 'assets\imageAnimations\\black')
