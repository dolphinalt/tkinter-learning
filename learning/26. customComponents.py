...
class Segment(ttk.Frame):
    def __init__(self, parent, label_text, button_text, exercise_text):
        ...
        self.exercise(exercise_text).grid(row = 0, column = 2, sticky = 'nsew')

    def exercise(self, button_text):
...
# window
window = tk.Tk()
window.title('Widgerts and return')
...
Segment(window, 'test', 'click', '123')
Segment(window, 'hello', 'test', 'test2')
Segment(window, 'bye', 'launch', '456')
Segment(window, 'last one', 'exit', 'test3')
